...
?>
        ...
        <?php foreach ($all_stats as $dept): ?>
            <tr>
                ...
                <td style="text-align: center;">
                    <span style="padding: 0.25rem 0.75rem; background: #e9ecef; color: white; border-radius: 5px;">
    ...
    <div style="margin-top: 2rem;">
        ...
        <div style="background: white; padding: 1.5rem; border-radius: 8px; margin: 1rem 0; border-left: 3px solid #667eea;">
            <h4>Chart Gaji Rata-rata per Departemen</h4>
            <?php foreach ($all_stats as $dept): ?>
                <div style="display: flex; justify-content: space-between; margin-bottom: 0.5rem;">
                    <span><?php echo htmlspecialchars($dept['department']); ?></span>
                    <span>Rp <?php echo number_format($dept['avg_salary'], 0, ',', '.'); ?></span>
                </div>
                <div style="background: #f0f0f0; border-radius: 4px; height: 20px; margin-bottom: 0.5rem;">
        ...
        <div style="background: white; padding: 1.5rem; border-radius: 8px; margin: 1rem 0; border-left: 3px solid #27ae60;">
            <h4>Chart Jumlah Karyawan per Departemen</h4>
            <?php foreach ($all_stats as $dept): ?>
                <div style="display: flex; justify-content: space-between; margin-bottom: 0.5rem;">
                    <span><?php echo htmlspecialchars($dept['department']); ?></span>
                ...
                <div style="background: #f0f0f0; border-radius: 4px; height: 20px;">
                    <div style="background: #27ae60; border-radius: 4px; height: 100%; width: <?php echo ($dept['total_employees'] / max(array_column($all_stats, 'total_employees')) * 100); ?>%;"></div>
                </div>
            <?php endforeach; ?>
        </div>
    </div>
...
<div style="margin-top: 2rem; padding: 1rem; background: #fff3cd; border-radius: 5px;">
    ...
    <p>Halaman ini generate secara real-time dari VIEW PostgreSQL yang menggunakan fungsi agregat seperti <code>COUNT()</code>, <code>AVG()</code>, <code>MIN()</code>, <code>MAX()</code>, dan <code>SUM()</code>.</p>
</div>
